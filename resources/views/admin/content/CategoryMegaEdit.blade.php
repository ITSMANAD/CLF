    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">ویرایش مگا منو</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-left">
                            ...
                            <li class="breadcrumb-item">{{$megacategory['name']}}</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
                ...
                <div class="container">
                    ...
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">{{$megacategory['name']}}</h3>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form method="post" action="{{route('CategoryAddMegaOne')}}" role="form">
                            @method('patch')
                            @csrf
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="nameinput">اسم مگا دسته بندی</label>
                                    <input type="text" name="name" class="form-control" id="nameinput" value="{{$megacategory['name']}}">
                                </div>
                                <div class="form-group">
                                    <label for="nameinput">آدرس مگا دسته بندی</label>
                                    <input type="text" name="slug" class="form-control" id="nameinput" value="{{$megacategory['slug']}}">
                                </div>
                                <div class="form-group">
                                    @php
                                        $categoryid = $megacategory['category'];
                                        $CategorySelect = \App\Models\Category::all()->whereIn('id',$categoryid);

                                    @endphp
                                    <label for="nameinput">   دسته بندی اصلی </label>
                                    <select name="category" class="form-control">
                                        @foreach($categories as $category)
                                            @foreach($CategorySelect as $dfsrg)
                                                <option @if($category['id']==$dfsrg['id']) selected @else @endif  value="{{$category['id']}}">{{$category['name']}}</option>
                                ...
                                <div class="form-group">
                                    ...
                                    @php
                                        $categoryid = $megacategory['subcategory'];
                                        $CategorySelect = \App\Models\SubCategory::all()->whereIn('id',$categoryid);

                                    @endphp
                                    <label for="nameinput">زیر دسته بندی مادر</label>
